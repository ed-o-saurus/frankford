...
The fit data returned by calling the fitter is a ``numpy`` array. This shape is determined by the :py:class:`frankford.Dataset` objects passed. Each element of the array represents one fit and consists of several fields.

Result
++++++

The ``result`` field is a ``np.int8`` whose value indicates the way that fit algorithm failed or succeeded. Negative values indicate failure while positive values indicate success. It can be interpreted by using it to construct a :py:class:`frankford.Result` object.
...
The ``parameters`` field is a collection of sub-fields of type ``np.double`` storing the optimized parameter values. Free parameters are set to the values that minimize :math:`\chi^2`. Fixed parameters are set to the values provided. Tied parameters are calculated from other parameters.
