...
// The C code is based on MP-fit
// (https://pages.physics.wisc.edu/~craigm/idl/cmpfit.html)
// developed by Craig Markwardt.
...
extern __device__ bool call_func(double *out, double *params, double *ind_vars,
                                 uint32_t i_dataset);
}

__device__ int8_t calculate(struct CallConfig *call_config, double *fvec,
                            double *qtf, double *params_all, double *params,
                            double *params_new, double *fjac, double *diag,
                            double *wa1, double *wa2, double *wa3, double *wa4,
                            int64_t *ipvt, double *ind_vars, double *fixed);
__device__ struct Point *read_dataset(struct CallConfig *call_config,
                                      int64_t i_point, struct Dataset *dataset,
                                      double *ind_vars);
__device__ float read_param(struct CallConfig *call_config,
                            struct ParamArray *param_array);
__device__ bool get_deviates(struct CallConfig *call_config, double *fvec,
                             double *params, double *ind_vars);
__device__ bool fdjac(struct CallConfig *call_config, double *fjac,
                      double *params, double *fvec, double *ind_vars,
                      double *wa1, double *wa2, double *wa4);
__device__ void qrfac(struct CallConfig *call_config, double *a, int64_t *ipvt,
                      double *rdiag, double *acnorm, double *wa);
__device__ void lmpar(struct CallConfig *call_config, double *r, int64_t *ipvt,
                      double *diag, double *qtb, double delta, double &par,
                      double *x, double *sdiag, double *wa3, double *wa4);
__device__ void qrsolv(struct CallConfig *call_config, double *r, int64_t *ipvt,
                       double *diag, double *qtb, double *x, double *sdiag,
                       double *wa);
__device__ void covar(struct CallConfig *call_config, double *r, int64_t *ipvt,
                      double *wa);
...
fit(int32_t *_, // Unused return value
    int64_t i_thread, int64_t n_free_param, struct FreeParam **free_params,
    int64_t n_fixed_param, struct ParamArray **fixed_params,
    int64_t n_tied_param, int64_t n_dataset, struct Dataset **datasets,
    double ftol, double xtol, double gtol, double stepfactor, double covtol,
    int64_t maxiter, uint8_t douserscale, uintptr_t result_offset,
    uintptr_t chi_sq_offset, uintptr_t dof_offset, uintptr_t num_iter_offset,
    uintptr_t orig_chi_sq_offset, int64_t n_returned_param,
    struct PositionOffset **returned_params, int64_t n_returned_uncertainty,
    struct PositionOffset **returned_uncertainties, int64_t n_returned_covar,
    struct PositionsOffset **returned_covars, uintptr_t ret_value, double *fvec,
    double *qtf, double *params_all, double *params, double *params_new,
    double *fjac, double *diag, double *wa1, double *wa2, double *wa3,
    double *wa4, int64_t *ipvt, double *ind_vars, double *fixed) {
  *_ = 0;
  ...
  uint8_t result =
      calculate(&call_config, fvec, qtf, params_all, params, params_new, fjac,
                diag, wa1, wa2, wa3, wa4, ipvt, ind_vars, fixed);
  ...
  *((int8_t *)(ret_value + result_offset)) = result;
  ...
  if (result > 0) {
    ...
    *((int64_t *)(ret_value + dof_offset)) = m - n_free_param;
    ...
    memcpy(params + call_config.n_free_param, fixed,
           call_config.n_fixed_param * sizeof(double));
    ...
    for (int64_t i_returned_param = 0; i_returned_param < n_returned_param;
         i_returned_param++) {
      struct PositionOffset *returned_param = returned_params[i_returned_param];
      *((double *)(ret_value + returned_param->offset)) =
          params[returned_param->position];
    }

    if (n_returned_uncertainty > 0 || n_returned_covar > 0) {
      covar(&call_config, fjac, ipvt, wa2);

      for (int64_t i_returned_uncertainty = 0;
           i_returned_uncertainty < n_returned_uncertainty;
           i_returned_uncertainty++) {
        struct PositionOffset *returned_uncertainty =
            returned_uncertainties[i_returned_uncertainty];
        double cc = fjac[(returned_uncertainty->position) * (m + 1)];

        *((double *)(ret_value + returned_uncertainty->offset)) =
            (cc > zero) ? sqrt(cc) : zero;
      }

      for (int64_t i_returned_covar = 0; i_returned_covar < n_returned_covar;
           i_returned_covar++) {
        struct PositionsOffset *returned_covar =
            returned_covars[i_returned_covar];
        *((double *)(ret_value + returned_covar->offset)) =
            fjac[(returned_covar->position1) * m + (returned_covar->position2)];
      }
    }
  }
...
__device__ int8_t calculate(struct CallConfig *call_config, double *fvec,
  ...
  for (int64_t j = 0; j < call_config->n_free_param; j++)
    params_all[j] =
        read_param(call_config, call_config->free_params[j]->init_value);

  for (int64_t j = 0; j < call_config->n_fixed_param; j++)
    fixed[j] = params_all[call_config->n_free_param + j] =
        read_param(call_config, call_config->fixed_params[j]);
  ...
  while (true) // start of outer loop
  {
    ...
    for (int64_t j = 0; j < call_config->n_free_param; j++) {
      ...
      bool lpegged =
          params[j] ==
          read_param(call_config, call_config->free_params[j]->lower);
      bool upegged =
          params[j] ==
          read_param(call_config, call_config->free_params[j]->upper);
      float sum = zero;

      // If the parameter is pegged at a limit, compute the gradient direction
      if (lpegged || upegged)
        for (int64_t i = 0, ij = j * call_config->m; i < call_config->m;
             i++, ij++)
          sum += fvec[i] * fjac[ij];

      // If pegged at lower limit and gradient is toward negative then reset
      // gradient to zero If pegged at upper limit and gradient is toward
      // positive then reset gradient to zero
    ...
    for (int64_t j = 0, jj = 0; j < call_config->n_free_param;
         j++, jj += (call_config->m) + 1) {
    ...
    // (From this point on, only the square matrix, consisting of the triangle
    // of R, is needed.)
    ...
    do // beginning of the inner loop.
    {
      ...
      for (int64_t j = 0; j < call_config->n_free_param; j++) {
        ...
        // Adjust the output values.  If the step put us exactly on a boundary,
        // make sure it is exact.
        sgnu = (upper >= 0) ? (+1) : (-1);
        sgnl = (lower >= 0) ? (+1) : (-1);
        ulim1 = upper * (1 - sgnu * DBL_EPSILON) -
                ((upper == 0) ? (DBL_EPSILON) : 0);
        llim1 = lower * (1 + sgnl * DBL_EPSILON) +
                ((lower == 0) ? (DBL_EPSILON) : 0);
      ...
      memcpy(params_new + call_config->n_free_param, fixed,
             call_config->n_fixed_param * sizeof(double));
      ...
      double temp1 =
          norm(call_config->n_free_param, wa3) * alpha / call_config->fnorm;
      ...
      bool ok_chi_sq = (fabs(actred) <= call_config->ftol) &&
                       (prered <= call_config->ftol) && (p5 * ratio <= one);
...
__device__ struct Point *read_dataset(struct CallConfig *call_config,
  ...
  for (int64_t i_ind_var = 0; i_ind_var < dataset->n_ind_vars; i_ind_var++) {
    ...
    ind_vars[i_ind_var] =
        ind_var->values[ind_var_out_offset + ind_var_fit_offset];
  }
...
__device__ bool get_deviates(struct CallConfig *call_config, double *fvec,
  ...
  for (int64_t i_dataset = 0; i_dataset < call_config->n_dataset; i_dataset++) {
    ...
    for (int64_t i_point = 0; i_point < dataset->fit_size; i_point++) {
      struct Point *point =
          read_dataset(call_config, i_point, dataset, ind_vars);
      if (!isfinite(point->value))
        continue;
...
__device__ bool fdjac(struct CallConfig *call_config, double *fjac,
  ...
  if (center_needed) {
    m = 0;
    for (int64_t i_dataset = 0; i_dataset < call_config->n_dataset;
         i_dataset++) {
      struct Dataset *dataset = call_config->datasets[i_dataset];
...
__device__ void qrfac(struct CallConfig *call_config, double *a, int64_t *ipvt,
  ...
  for (int64_t j = 0, ij = 0; j < call_config->n_free_param;
       j++, ij += call_config->m) {
  ...
  for (int64_t j = 0; j < call_config->n_free_param; j++) {
    ...
    if (kmax != j) {
      for (int64_t i = 0, ij = (call_config->m) * j,
                   jj = (call_config->m) * kmax;
           i < call_config->m; i++, ij++, jj++) {
        // SWAP
        double temp = a[ij];
        a[ij] = a[jj];
        a[jj] = temp;
    ...
    if (ajnorm != zero) {
      ...
      if (jp1 < call_config->n_free_param)
        for (int64_t k = jp1; k < call_config->n_free_param; k++) {
          double sum = zero;
          for (int64_t i = j, ij = j + (call_config->m) * k,
                       jj = j + (call_config->m) * j;
               i < call_config->m; i++, ij++, jj++)
            sum += a[jj] * a[ij];
          ...
          if (rdiag[k] != zero) {
            ...
            if ((p05 * temp * temp) <= DBL_EPSILON) {
              rdiag[k] = norm((call_config->m) - j - 1,
                              a + jp1 + (call_config->m) * k);
              wa[k] = rdiag[k];
            }
          }
        }
...
__device__ void lmpar(struct CallConfig *call_config, double *r, int64_t *ipvt,
  ...
  for (int64_t iter = 0; true; iter++) {
    ...
    if ((fabs(fp) <= p1 * delta) ||
        ((parl == zero) && (fp <= temp) && (temp < zero)) || (iter == 9))
      return;
    ...
    for (int64_t j = 0, jj = 0; j < call_config->n_free_param;
      ...
      int64_t jp1 = j + 1;
      if (jp1 < call_config->n_free_param)
        for (int64_t i = jp1, ij = jp1 + jj; i < call_config->n_free_param;
             i++, ij++)
          wa3[i] -= r[ij] * temp;
    }
...
__device__ void qrsolv(struct CallConfig *call_config, double *r, int64_t *ipvt,
  ...
  for (int64_t j = 0, kk = 0; j < call_config->n_free_param;
       j++, kk += (call_config->m) + 1) {
    for (int64_t i = j, ij = kk, ik = kk; i < call_config->n_free_param;
         i++, ij++, ik += call_config->m)
      r[ij] = r[ik];
    ...
    if (diag[l] != zero) {
      ...
      for (int64_t k = j; k < call_config->n_free_param; k++) {
        ...
        if (call_config->n_free_param > k + 1)
          for (int64_t i = k + 1, ik = kk + 1; i < call_config->n_free_param;
               i++, ik++) {
  ...
  for (int64_t k = 0; k < nsing; k++) {
    ...
    double sum = zero;
    if (nsing > j + 1)
      for (int64_t i = j + 1, ij = j + 1 + (call_config->m) * j; i < nsing;
           i++, ij++)
        sum += r[ij] * wa[i];
